...
public class Adapter_Link extends ArrayAdapter<Links> {

    int ctrl;
    public Adapter_Link(@NonNull Context context, ArrayList<Links> splink) {

        super(context, 0,splink);
    }
    ...
    public View iniView(int position, View convertView, ViewGroup parent) {
        ...
        Links currentItem = getItem(position);
        if(currentItem!=null) {
            txtitem.setText(currentItem.getNomelink());
        }
        return convertView;
    }
}
